...
async function main() {
  // Scénario exact toast utilisateur : L2 ANDRANA, L3-L4 doublons, erreur materialKey
  const rows = [
    {
      Matière: 'ANDRANA',
  ...
  const row = await prisma.baseMaterial.findFirst({
    where: { OR: [{ id: ID }, { materialKey: { contains: 'acrylic' } }] },
  ...
  console.log(JSON.stringify({
    ok,
    report: { updated: report.updated, errors: report.errors, duplicateIds: report.duplicateIds },
    row: row ? { id: row.id, label: row.label, price: row.basePrintPrice, key: row.materialKey } : null,
  }, null, 2));
  process.exit(ok ? 0 : 1);
}
