...
import { importDesignFromExcel } from '@/lib/server/modules/direct-sale/pricing-tables.service';

export const POST = withAuthApi('design import', async (auth, req) => {
  try {
    ...
    return NextResponse.json({ ok: true, data: await importDesignFromExcel(rows, auth) });
